...
function bilateracion() {
    ...
    if (a.radio > disBeacons) {
        ...
        if (b.radio > disBeacons) {
            ...
            if (a.radio + b.radio < disBeacons) {
                //los radios no se intersectan
                refactorizarRadios(a, b, disBeacons);


            } else {
                //Radios correctos
                intersect2beacons(a, b, disBeacons);
            }
...
function refactorizarRadios(a, b, distancia) {

    var dif = difRadiosDist(a, b, distancia);
    a.radio = a.radio + dif;
    b.radio = b.radio + dif;
...
function intersect2beacons(a, b, dist) {
    ...
    var radio2 = Math.sqrt((a.radio * a.radio) - (p * p));


    dibujarRadioPosicion(pX, pY, redondeo(radio2) * escala);
...
function trilateracion() {
    ...
    if (p4 !== null) {
        if (p4 instanceof Array) {
            ...
            */
            alert("0 " + p4[0].posX + " - " + p4[0].posY);
            //context.fillText(p4[0].x + " - " + p4[0].y,p4[0].x-35, p4[0].y+20);
            /*
            ctx.fillStyle = "#ff0";
            ctx.fillRect(20 + p4[1].x - 2, 20 + p4[1].y - 2, 5, 5);
            */
            alert("1 " + p4[1].posX + " - " + p4[1].posY);
            //context.fillText(p4[1].x + " - " + p4[1].y,p4[1].x-35, p4[1].y+20);
        }
        else {
            /*
            ctx.fillStyle = "#fff";
            ...
            */
            alert("UNICO " + p4.posX + " - " + p4.posY);
            //context.fillText(p4.x + " - " + p4.y,p4.x-35, p4.y+20);
        }
    } else {

        var disBeacons = dist2puntos(a.posX, a.posY, b.posX, b.posY);
        if (a.radio + b.radio < disBeacons) {
            var dif = difRadiosDist(a, b, disBeacons);
            a.radio = a.radio + dif;
            b.radio = b.radio + dif;

        }
        var disBeacons2 = dist2puntos(a.posX, a.posY, c.posX, c.posY);
        if (a.radio + c.radio < disBeacons2) {
            var dif = difRadiosDist(a, c, disBeacons2);
            a.radio = a.radio + dif;
            c.radio = c.radio + dif;
        ...
        if (p4 !== null) {
            ...
            dibujarRadioPosicion(p4[0].posX, p4[0].posY, redondeo(a.radio) * escala);
            }else{
                alertarOns("Atención:", "Refact único");
            dibujarRadioPosicion(p4.posX, p4.posY, redondeo(a.radio) * escala);
            }
        } else {

            alertarOns("Atención:", "Sin solución");
            dibujarRadioPosicion(a.posX, a.posY, redondeo(a.radio) * escala);
            dibujarRadioPosicion(b.posX, b.posY, redondeo(b.radio) * escala);
            dibujarRadioPosicion(c.posX, c.posY, redondeo(c.radio) * escala);
